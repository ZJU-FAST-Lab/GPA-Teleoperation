...
std::pair<double, double> calculate_yaw(double rc_yaw_dot, double dt)
{
  constexpr double YAW_DOT_MAX_PER_SEC = 2 * M_PI;
  constexpr double YAW_DOT_DOT_MAX_PER_SEC = 5 * M_PI;
  std::pair<double, double> yaw_yawdot(0, 0);
...
void RCYawCallback( const geometry_msgs::PointStampedConstPtr &msg )
{
  rc_yaw_dot_ = msg->point.x;
}
...
std::pair<double, double> calculate_yaw_rc(double rc_yaw_dot, double dt)
{
  constexpr double YAW_DOT_MAX_PER_SEC = 1.5 * M_PI;
  std::pair<double, double> yaw_yawdot(0, 0);

  if( rc_yaw_dot > YAW_DOT_MAX_PER_SEC )
  {
    rc_yaw_dot = YAW_DOT_MAX_PER_SEC;
  }
  else if( rc_yaw_dot < -YAW_DOT_MAX_PER_SEC )
  {
    rc_yaw_dot = -YAW_DOT_MAX_PER_SEC;
  }

  double yawdot = 0;
  double d_yaw = rc_yaw_dot * dt;
...
void cmdCallback(const ros::TimerEvent &e)
{
  ...
  if ((time_now - heartbeat_time_).toSec() > 5.0)
  {
  ...
  if (t_cur < traj_duration_ && t_cur >= 0.0)
  {
    ...
    /*** calculate yaw ***/
    if(traj_server_rc_yaw_)
      yaw_yawdot = calculate_yaw_rc(rc_yaw_dot_, (time_now - time_last).toSec());
    else
  ...
  if( rc_yaw_dot_ != 0.0 )
  {
    ...
    if (t_cur >= traj_duration_ && t_cur >= 0.0)
    {
      publish_cmd(last_pos_, Vector3d::Zero(), Vector3d::Zero(), Vector3d::Zero(), yaw_yawdot.first, yaw_yawdot.second);
    }
  }
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber rc_yaw_dot_sub = nh.subscribe("/intention_capture/rc_yaw_dot", 10, RCYawCallback);
  ...
  nh.param("traj_server/traj_server_rc_yaw", traj_server_rc_yaw_, true);
  nh.param("traj_server/static_debug", static_debug_, false);
