...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    nh.param("manager/max_vel", pp_.max_vel_param_, -1.0);
    pp_.max_vel_ = pp_.max_vel_param_;
    ...
    last_local_target_pt_ = Eigen::Vector3d{0, 0, 0};
  }
      ...
      poly_traj::MinJerkOpt &initMJO)
  {
    ...
    last_local_target_pt_ = local_target_pt;

    // if (flag_first_call || flag_polyInit) /*** case 1: polynomial initialization ***/
    if (true) /*** case 1: polynomial initialization ***/
    {
      ...
      Eigen::Vector3d temp_vec = tailState.col(0) - headState.col(0);
      double temp_vec_length = temp_vec.norm();
      temp_vec.normalize();
      for( int i = 0; i < piece_nums-1; i++ )
      {
        innerPs.col(i) = headState.col(0) + (double)i/((double)(piece_nums-1)) * temp_vec_length * temp_vec;
      }

      piece_dur_vec.resize(piece_nums);
      double piece_dur_init = init_of_init_totaldur / (double)piece_nums;
      piece_dur_vec = Eigen::VectorXd::Constant(piece_nums, piece_dur_init);
    ...
    else /*** case 2: initialize from previous optimal trajectory ***/
    {
      ...
      //找到轨迹上距离topo点最近的点的后一个点,拿过来当做innerPs
      double passed_t_on_lctraj = ros::Time::now().toSec() - traj_.local_traj.start_time;
      double t_start_to_end = traj_.local_traj.duration - passed_t_on_lctraj;
      double length_start_to_end = (start_pt - last_local_target_pt_).norm();
      double length_end_to_tg = (local_target_pt - last_local_target_pt_).norm();
      if( length_end_to_tg > 0.2 )
      {
        std::cout << "111" << std::endl;
        std::cout << length_end_to_tg << std::endl;

        double t_end_to_tg = t_start_to_end / length_end_to_tg * length_start_to_end;
        ...
        Eigen::VectorXd piece_dur_vec = Eigen::VectorXd::Constant(piece_nums, (t_end_to_tg + t_start_to_end) / piece_nums);
        headState << start_pt, start_vel, start_acc;
        tailState << local_target_pt, local_target_vel, Eigen::Vector3d::Zero();
        ...
        for (int i = 0; i < piece_nums - 1; ++i)
        {
          ...
          {
            double glb_t = t - t_start_to_end;
            Eigen::Vector3d vec = local_target_pt - last_local_target_pt_;
            innerPs.col(i) = (glb_t/t_end_to_tg) * vec + last_local_target_pt_;
          }
      ...
      else
      {
        ...
        Eigen::Matrix3d headState, tailState;
        Eigen::MatrixXd innerPs(3, piece_nums - 1);
        Eigen::VectorXd piece_dur_vec = Eigen::VectorXd::Constant(piece_nums, t_start_to_end / piece_nums);
        headState << start_pt, start_vel, start_acc;
        tailState << local_target_pt, local_target_vel, Eigen::Vector3d::Zero();
  ...
  bool EGOPlannerManager::computeInitState_zx(
      const Eigen::Vector3d &start_pt, const Eigen::Vector3d &start_vel, const Eigen::Vector3d &start_acc,
      const Eigen::Vector3d &local_target_pt, const Eigen::Vector3d &local_target_vel,
      const bool flag_polyInit, const bool flag_randomPolyTraj, const double &ts,
      poly_traj::MinJerkOpt &initMJO)
  {
    ...
    // if (true) /*** case 1: polynomial initialization ***/
    {
      ...
      // ploy_traj_opt_->calculateAStar(simple_path, start_pt, local_target_pt);
  ...
  void EGOPlannerManager::updatePlanParam( double vel )
  {
    if( vel > pp_.max_vel_param_ )
      vel = pp_.max_vel_param_;
    pp_.max_vel_ = vel + 0.1;
    ploy_traj_opt_->updateOptimizationParam( vel );
  }

  void EGOPlannerManager::updateYaw( double yaw )
  {
    Eigen::Vector3d yaw_direction;
    yaw_direction[0] = cos(yaw);
    yaw_direction[1] = sin(yaw);
    yaw_direction[2] = 0.0;
    ploy_traj_opt_->updateYawDirection(yaw_direction);
  }

  void EGOPlannerManager::updatePlanParam( Eigen::Vector3d racing_ring )
  {
    racing_ring_ = racing_ring;
  }

  void EGOPlannerManager::updateGoal( Eigen::Vector3d odom, Eigen::Vector3d goal )
  {
    ploy_traj_opt_->updateOptimizationGoal( odom, goal );
  }
  void EGOPlannerManager::updateTopoPoint( Eigen::Vector3d topo_point )
  {
    topo_point_ = topo_point;
    ploy_traj_opt_->updateTopoPoint( topo_point );
  }
