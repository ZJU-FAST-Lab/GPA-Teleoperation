...
#include "optimizer/lbfgs_lite.hpp"
...
namespace ego_planner
{
  ...
  public:
    ...
    int topo_point_cp_id;
    TopoConstraint topo_constraint;
  ...
  private:
    ...
    double wei_safe_vel_;
    double wei_visibility_;
    double wei_visibility_param_;
    double wei_length_;
    double wei_topo_;
    ...
    Eigen::Vector3d yaw_direction_norm_;
    Eigen::Vector3d goal_;
    std::vector<bool> first_flag_;
    std::vector<double> first_length_;
  ...
  public:
    ...
    void calculateAStar(std::vector<Eigen::Vector3d> &simple_path, Eigen::Vector3d start_pos, Eigen::Vector3d end_pos);
    ...
    // ! wxx
    void updateOptimizationParam( double vel );
    void updateYawDirection( Eigen::Vector3d yaw_direction );
    void updateOptimizationGoal( Eigen::Vector3d odom, Eigen::Vector3d goal );
    void updateTopoPoint( Eigen::Vector3d topo_point );
    void roughlyCheckTopoConstraint();
  ...
  private:
    ...
    bool topoGradCostP(const int i_dp,
                       const Eigen::Vector3d &p,
                       Eigen::Vector3d &gradp,
                       double &costp);

    bool visibilityGradCostP(const int i_dp,
                             const Eigen::Vector3d &p,
                             Eigen::Vector3d &gradp,
                             double &costp);
    ...
    bool safeVelocityGradCostV(const int i_dp,
                               const Eigen::Vector3d &v,
