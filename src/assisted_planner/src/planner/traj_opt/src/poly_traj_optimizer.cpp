...
namespace ego_planner
{
      ...
      Eigen::MatrixXd &optimal_points, double &final_cost)
  {
    ...
    do
    {
      ...
      int flag_size = 6;
      first_flag_.resize(flag_size);
      first_length_.resize(flag_size);
      for( int ii = 0; ii < flag_size; ii++ )
        first_flag_[ii] = true;
      /* ---------- optimize ---------- */
      roughlyCheckTopoConstraint();
      t1 = ros::Time::now();
  ...
  void PolyTrajOptimizer::roughlyCheckTopoConstraint()
  {
    cps_.topo_point_cp_id = -1;
    ...
    int i_end = cps_.points.cols() - 1;
    double min_length = 100.0;
    int min_id;
    for( int i = i_end; i > 0; i-- )
    {
      double length = ( cps_.points.col(i) - cps_.topo_constraint.topo_centre ).norm();
    ...
    // if( min_length > 0.5 )
    //   ROS_ERROR("This Topo constraint is bad!!!");

    Eigen::Vector3d vec = cps_.topo_constraint.topo_centre - cps_.points.col(0);
    double vec_length = vec.norm();
    vec.normalize();
    if( vec.dot( cps_.points.col(min_id) ) < vec_length )
    {
      if( min_id+1 < i_end )
      {
        double length = ( cps_.points.col(min_id+1) - cps_.topo_constraint.topo_centre ).norm();
        if( length < 0.3 )
        {
          min_id += 1;
        }
  ...
  void PolyTrajOptimizer::addPVAGradCost2CT(EIGENVEC &gdT, Eigen::VectorXd &costs, const int &K)
  {
    ...
    // Penalize the initial jerk magnitude because fore frequent replan
    Eigen::Matrix<double, 6, 1> beta3Ini;
    beta3Ini.setZero();
    beta3Ini(3) = 6.0;
    Eigen::Vector3d iniJer = jerkOpt_.get_b().block<6, 3>(0, 0).transpose() * beta3Ini;
    double costj;
    Eigen::Vector3d gradj;
    if (feasibilityGradCostJ(iniJer, 30.0, 1000.0, gradj, costj))
    {
      jerkOpt_.get_gdC().block<6, 3>(0, 0) += beta3Ini * gradj.transpose();
      costs(2) += costj;
    }
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        // topo
        if (topoGradCostP(i_dp, pos, gradp, costp))
        {
          gradViolaPc = beta0 * gradp.transpose();
          gradViolaPt = alpha * gradp.transpose() * vel;
          jerkOpt_.get_gdC().block<6, 3>(i * 6, 0) += omg * step * gradViolaPc;
        ...
        // visibility
        if (visibilityGradCostP(i_dp, pos, gradp, costp))
        {
          gradViolaPc = beta0 * gradp.transpose();
          gradViolaPt = alpha * gradp.transpose() * vel;
          jerkOpt_.get_gdC().block<6, 3>(i * 6, 0) += omg * step * gradViolaPc;
        ...
        // safe velocity
        // if (safeVelocityGradCostV(i_dp, vel, gradv, costv))
        // {
  ...
  bool PolyTrajOptimizer::visibilityGradCostP(const int i_dp,
                                              ...
                                              double &costp)
  {
    ...
    Eigen::Vector3d goal_pos = goal_;
    Eigen::Vector3d now_pos = p;
    
    if( first_flag_[i_dp-1] )
    {
      first_flag_[i_dp-1] = false;
      first_length_[i_dp-1] = (now_pos - goal_pos).norm();
    }

    double length_costp = 0;
    Eigen::Vector3d length_gradp;
    length_gradp.setZero();
    double norm_length = (now_pos - goal_pos).norm();
    double thr_length = first_length_[i_dp-1];
    if( norm_length < thr_length )
    {
      length_costp = pow( thr_length - norm_length, 3 );
      length_gradp = - 3 * ( thr_length - norm_length ) * ( thr_length - norm_length ) * (now_pos - goal_pos) / norm_length;
    }

    int sample = 20;
    double dist_threshold = 1.0;
    double line_norm = ( now_pos - goal_pos ).norm();
    ...
    for( int kk = 1; kk <= sample; kk++ )
    {
      ...
      Eigen::Vector3d pk = lambda_k * now_pos + ( 1- lambda_k ) * goal_pos;
      ...
      {
        double line_length = ( 1- lambda_k ) * line_norm;
        costp += pow( (dist_threshold - dist), 3 );
        gradp = gradp - 3 * (dist_threshold-dist) * (dist_threshold-dist) * 
                ( dist_grad * lambda_k );
      }
    ...
    costp = costp * wei_visibility_ + length_costp * wei_length_;
    gradp = gradp * wei_visibility_ + length_gradp * wei_length_;

    return true;
  }

  bool PolyTrajOptimizer::topoGradCostP(const int i_dp,
                                        const Eigen::Vector3d &p,
                                        Eigen::Vector3d &gradp,
                                        double &costp)
  {
    if (i_dp != cps_.topo_point_cp_id)
      return false;
    ...
    // Topo cost
    Eigen::Vector3d temp_vec = p - cps_.topo_constraint.topo_centre;
    double dist = temp_vec.norm();
    double dist_err = dist - cps_.topo_constraint.radius;

    if( dist_err > 0 )
    {
      ret = true;
      costp += wei_topo_ * pow(dist_err, 3);
      gradp += wei_topo_ * 3.0 * dist_err * dist_err * temp_vec / dist;
    }
                                            ...
                                            double &costp)
  {
    ...
    // double dist;
    // Eigen::Vector3d dist_grad;
    // grid_map_->evaluateESDFWithGrad( p, dist, dist_grad );
    // double dist_err = obs_clearance_ - dist;
    ...
    // std::cout << "collision costp:" << costp << std::endl;
    // std::cout << "collision gradp:" << gradp.transpose() << std::endl;
  ...
  bool PolyTrajOptimizer::safeVelocityGradCostV(const int i_dp,
                                                const Eigen::Vector3d &v,
                                                ...
                                                double &costv)
  {
    // return 0;
    // if (i_dp <= 0 || i_dp > ConstraintPoints::two_thirds_id(cps_.points, touch_goal_)) // only apply to first 2/3
    if (i_dp <= 0 || i_dp > 5) // only apply to first 2/3
      return false;

    // 用当前的yaw的方向算一个点乘
    Eigen::Vector3d vel = v;
    vel[2] = 0;
    Eigen::Vector3d vel_temp = vel;
    double vel_norm = vel.norm();
    if( vel_norm < 1e-2 )
      return false;
    vel_temp.normalize();
    double dot_cost = yaw_direction_norm_.dot(vel_temp);
    double vel_cost = 0.85 - dot_cost;
    if (vel_cost > 0)
    {
      gradv = wei_safe_vel_ * 3 * vel_cost * vel_cost * ( dot_cost * vel - yaw_direction_norm_ * vel_norm ) / ( vel_norm * vel_norm );
      costv = wei_safe_vel_ * vel_cost * vel_cost * vel_cost;
      return true;
    }
    return false;
  }
  ...
  bool PolyTrajOptimizer::feasibilityGradCostJ(const Eigen::Vector3d &j,
                                               const double &jmax,
                                               const double &weight_j,
                                               Eigen::Vector3d &gradj,
                                               double &costj)
  {
    double jpen = j.squaredNorm() - jmax * jmax;
    if (jpen > 0)
    {
      gradj = weight_j * 6 * jpen * jpen * j;
      costj = weight_j * jpen * jpen * jpen;
      return true;
    }
    return false;
  }
  ...
  void PolyTrajOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    nh.param("optimization/weight_safe_vel", wei_safe_vel_, -1.0);
    nh.param("optimization/weight_visibility", wei_visibility_param_, -1.0);
    nh.param("optimization/weight_length", wei_length_, -1.0);
    nh.param("optimization/weight_topo", wei_topo_, -1.0);
    ...
    // yaw_direction_norm_ = Eigen::Vector3d{1, 0, 0};
  ...
  void PolyTrajOptimizer::updateOptimizationParam(double vel)
  {
    // if( vel > 1.0 )
    //   wei_time_ =
    max_vel_ = vel + 0.1;
  ...
  void PolyTrajOptimizer::updateYawDirection( Eigen::Vector3d yaw_direction )
  {
    yaw_direction.normalize();
    yaw_direction_norm_ = yaw_direction;
  }
  void PolyTrajOptimizer::updateOptimizationGoal( Eigen::Vector3d odom, Eigen::Vector3d goal )
  {
    goal_ = goal;
    wei_visibility_ = wei_visibility_param_;

    // if( ( odom[2] - goal[2] ) > 0.5 )
    //   wei_visibility_ = 0.0;
  }
  void PolyTrajOptimizer::updateTopoPoint( Eigen::Vector3d topo_point )
  {
    // calculateTopo(topo_point);
    cps_.topo_constraint.topo_centre = topo_point;
    double dist;
    grid_map_->evaluateESDF( topo_point, dist );
    if( dist > 0.5 )
      dist = 0.5;
    cps_.topo_constraint.radius = dist;
    have_topo_point_ = true;
  }
  void PolyTrajOptimizer::calculateAStar(std::vector<Eigen::Vector3d> &simple_path, Eigen::Vector3d start_pos, Eigen::Vector3d end_pos)
  {
